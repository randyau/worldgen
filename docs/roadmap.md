...
- **M16 — Disasters, Reworked.** ✅ COMPLETE 2026-09-17 — see `docs/phases/archive/m16_disasters_reworked.md` (all phases 16.0–16.3 shipped same day). Wildfire/flood/earthquake/eruption now damage a co-located settlement's Health and roll a chance to destroy a co-located TileImprovement, reusing the war-raid damage → destroy-at-zero → RegisterRuin substrate (cause `"disaster"`) so the multi-year recovery arc falls out of the existing passive-Health-recovery mechanic for free. Volcanic ash got a finite duration (was indefinite) and now suppresses fertility while active, so ash-driven famine flows through the existing ResourcePressurePhase food ledger. Two new disaster types per the roadmap ask: Blight (settlement-scoped crop disease — destroys stored food, suppresses local fertility) and Harsh Winter (global — extra settlement decay + character Health drain). Balance sweep confirmed consequences fire meaningfully without ever reaching civilization-ending destruction at default config.
