...
- `Cooldown.cs` — Shared arithmetic and sentinels for the per-character cooldown stamps on WorldEngine.Sim.Entities.Characters.Tier1Character (, , , , ).
...
- `GoalTypeTraits.cs` — The single source of truth for which WorldEngine.Sim.Entities.Characters.GoalTypes are notable / long-running / discretionary / flourishing. Every enum value must have a row — see .
...
- `OrganizationMembership.cs` — The one place that mutates the two-sided membership invariant — WorldEngine.Sim.Entities.Characters.Tier1Character.Memberships and WorldEngine.Sim.Organizations.Organization.Members must always agree. M12 built for the civ case; M13–M15 then added nine ad-hoc join/leave/loyalty sites that updated both lists by hand. M15.9 routes them all here.
