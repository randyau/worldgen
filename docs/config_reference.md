...
| `reference_money_supply_per_capita` | `0.015` | `SimConfig.Economy.ReferenceMoneySupplyPerCapita` | Anchor for GlobalPriceIndex's EMA target: "what a fair per-capita money supply looks like." Tuned during 14.5 from long-run equilibrium data (3000-year, 3-seed instrumentation observed MoneySupplyPerCapita settling at 0.01-0.02 post-warm-up) — the original 50.0 placeholder pinned GlobalPriceIndex at price_index_min for the entire run. |
...
| `artifact_value_multiplier` | `0.35` | `SimConfig.Economy.ArtifactValueMultiplier` | Scalar for the 14.3 artifact-pricing formula (ArtifactBaseValue × this × GlobalPriceIndex). Lowered from the original placeholder 3.0 during 14.3's instrument-first pass to 1.5, then again during 14.5 to 0.35 after fixing GlobalPriceIndex's pinned-at-floor bug (see reference_money_supply_per_capita above): the index moved from a constant ~0.25 to floating near ~1.0 in the short run, a ~4x price jump that dropped ArtifactPurchased's fire rate from 3/5 to 1/5 seeds. Re-lowered proportionally so the purchase mechanic's reachability isn't an accidental casualty of an unrelated pricing fix. |
